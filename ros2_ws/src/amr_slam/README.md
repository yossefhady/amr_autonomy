...
The `amr_slam` package provides **Simultaneous Localization and Mapping (SLAM)** capabilities for the AMR (Autonomous Guided Vehicle) mobile robot using `slam_toolbox`. This package enables the robot to create maps of unknown environments while simultaneously tracking its position within those maps.
...
```bash
# From workspace root
cd /home/yossef/amr_autonomy/ros2_ws
...
```bash
# Disable RViz
ros2 launch amr_slam slam.launch.py use_rviz:=false

# Use custom parameters
ros2 launch amr_slam slam.launch.py slam_params_file:=/path/to/params.yaml

# Use custom RViz config
ros2 launch amr_slam slam.launch.py rviz_config:=/path/to/config.rviz

# Disable simulation time (for real robot)
ros2 launch amr_slam slam.launch.py use_sim_time:=false
```
...
```bash
# Disable RViz
ros2 launch amr_bringup slam_bringup.launch.py use_rviz:=false

# Disable joystick
ros2 launch amr_bringup slam_bringup.launch.py use_joystick:=false

# Spawn robot at different position
ros2 launch amr_bringup slam_bringup.launch.py x:=2.0 y:=3.0 yaw:=1.57

# Use different world
ros2 launch amr_bringup slam_bringup.launch.py world:=/path/to/world.sdf
```
...
5. **Save the map:**
   ...
   ros2 service call /slam_toolbox/serialize_map slam_toolbox/srv/SerializePoseGraph "{filename: '/home/yossef/amr_navigation/maps/my_warehouse_map'}"
   
   # Alternative: Save as PNG/YAML using map_server
   ros2 run nav2_map_server map_saver_cli -f /home/yossef/amr_navigation/maps/my_map
   ```
...
ls -la $(ros2 pkg prefix amr_slam)/share/amr_slam/rviz/
...
### With Navigation (amr_navigation)
...
ros2 launch amr_navigation navigation.launch.py map:=my_map.yaml
```

### With Real Hardware (amr_comm_bridge)
